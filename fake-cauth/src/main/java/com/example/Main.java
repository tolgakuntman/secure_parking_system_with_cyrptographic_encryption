...
/**
 * FAKE-CAUTH: A rogue Certificate Authority for security testing.
 * 
 * This service mimics the real CAuth but uses its own self-signed certificate chain.
 * It is used in negative test scenarios to verify that services properly reject
...
public class Main {
    ...
    // CA private key alias in keystore
    private static final String CA_KEY_ALIAS =
            System.getenv().getOrDefault("CA_KEY_ALIAS", "fake_cauth_server");
    ...
    public static void main(String[] args) {
        try {
            ...
            Log.info("FAKE-CAUTH starting (rogue CA for negative testing)");
            ...
            // Build SSLContext with KeyManagers (server identity) but NO TrustManagers
            ...
            try (SSLServerSocket serverSocket = (SSLServerSocket) factory.createServerSocket(PORT)) {
                ...
                // Don't require client certs for enrollment
                serverSocket.setNeedClientAuth(false);
                serverSocket.setWantClientAuth(false);

                Log.info("FAKE-CAUTH running on port " + PORT);
                Log.audit("FAKE-CAUTH cert subject: " + caCertificate.getSubjectX500Principal());
                Log.audit("FAKE-CAUTH cert issuer:  " + caCertificate.getIssuerX500Principal());
                Log.warn("This is a ROGUE CA - certificates issued here should be REJECTED by properly configured services");
            ...
            Log.secErr("FAKE-CAUTH fatal error: " + e.getMessage());
            e.printStackTrace();
            System.exit(1);
        }
    }
    ...
    private static class ClientHandler implements Runnable {
        ...
        public void run() {
            try (SSLSocket socket = clientSocket;
                ...
                Log.secErr("TLS handshake failed: " + hs.getMessage());
            } catch (Exception e) {
                Log.secErr("Client handler error: " + e.getMessage());
                e.printStackTrace();
            }
        }
    }
    ...
    private static String handleSignCSR(JsonObject req) {
        ...
        try {
            ...
            Log.info("FAKE-CAUTH signing CSR for: " + bcCsr.getSubject());
            Log.warn("Issuing certificate from UNTRUSTED rogue CA!");

            // Build leaf certificate signed by this FAKE CA
    ...
    private static void logPeerIdentity(SSLSocket socket) {
        try {
            ...
            Log.dbg("TLS: " + session.getProtocol() + " / " + session.getCipherSuite());
            Certificate[] peer = session.getPeerCertificates();
            if (peer.length > 0 && peer[0] instanceof X509Certificate x) {
                Log.audit("Peer subject: " + x.getSubjectX500Principal());
                Log.audit("Peer issuer:  " + x.getIssuerX500Principal());
            }
        } catch (Exception e) {
            Log.dbg("No peer certificate: " + e.getMessage());
        }
    }
