...
public class Log {
    ...
    static {
        SERVICE = System.getenv().getOrDefault("SERVICE_NAME", "CO");
    ...
    /**
     * Log mTLS peer from a single certificate with a label.
     */
    public static void auditPeer(X509Certificate cert, String label) {
        try {
            if (cert != null) {
                String cn = extractCN(cert);
                String fp = fingerprint(cert);
                audit(label + " peer CN=" + cn + " fp=" + fp);
            }
        } catch (Exception e) {
            dbg("Could not audit peer: " + e.getMessage());
        }
    ...
    /**
     * Extract CN from a distinguished name (DN) string.
     */
